...
export function useOfflineDriverRedirect() {
  ...
  useEffect(() => {
    const handleOnline = () => {
      console.log("[OfflineDriverRedirect] Connection restored");
      setIsOnline(true);
    };

    const handleOffline = () => {
      console.log("[OfflineDriverRedirect] Connection lost");
      setIsOnline(false);
    };
  ...
  useEffect(() => {
    if (!isOnline && isMobile && isDriver && !isOnAllowedPath) {
      console.log("[OfflineDriverRedirect] Redirecting driver to panel (offline + mobile)");
