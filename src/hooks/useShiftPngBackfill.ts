...
export function useShiftPngBackfill(enabled: boolean = true) {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const { data: shifts } = await supabase
          ...
          .select(
            "id, equipment_id, equipment_name, plate, driver_name, shift_end_time, final_horimeter, final_km, final_fuel_level"
          )
          .eq("shift_date", today)
    ...
    // Atrasa o primeiro tick para não competir com o boot do app.
