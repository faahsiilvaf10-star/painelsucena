...
export default function ConsumoAbastecimento() {
  ...
  const {
    data,
    isLoading
  } = useRefuelingData(selectedYear, selectedMonth);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Select value={selectedMonth.toString()} onValueChange={v => setSelectedMonth(parseInt(v))}>
              ...
              <SelectContent>
                {MONTH_NAMES.map((name, index) => <SelectItem key={index} value={index.toString()}>
                    {name}
                  </SelectItem>)}
              </SelectContent>
            </Select>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border-emerald-500/30">
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold mt-2">
                {(data?.refuelingByPoint?.find(p => p.point === "Ponto 3C")?.count || 0) + (data?.refuelingByPoint?.find(p => p.point === "Ponto 3D")?.count || 0)}
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              {data?.refuelingByPoint && data.refuelingByPoint.some(p => p.count > 0) ? <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={data.refuelingByPoint}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                    <XAxis dataKey="point" tick={{
                  fontSize: 12
                }} axisLine={false} />
                    <YAxis tick={{
                  fontSize: 12
                }} axisLine={false} allowDecimals={false} />
                    <Tooltip formatter={(value: number) => [`${value} abastecimentos`, "Quantidade"]} contentStyle={{
                  backgroundColor: "hsl(var(--card))",
                  border: "1px solid hsl(var(--border))",
                  borderRadius: "8px"
                }} />
                    <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                      {data.refuelingByPoint.map((entry, index) => <Cell key={`cell-${index}`} fill={POINT_COLORS[entry.point] || "#06b6d4"} />)}
                    </Bar>
                  </BarChart>
          ...
          <Card>
            ...
            <CardContent>
              {data?.refuelingByVehicle && data.refuelingByVehicle.length > 0 ? <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={data.refuelingByVehicle}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                    <XAxis dataKey="vehicleName" tick={{
                  fontSize: 12
                }} axisLine={false} />
                    <YAxis tick={{
                  fontSize: 12
                }} axisLine={false} tickFormatter={formatLiters} />
                    <Tooltip formatter={(value: number) => [`${value.toLocaleString("pt-BR")} L`, "Consumo"]} contentStyle={{
                  backgroundColor: "hsl(var(--card))",
                  border: "1px solid hsl(var(--border))",
                  borderRadius: "8px"
                }} />
                    <Bar dataKey="liters" radius={[4, 4, 0, 0]}>
                      {data.refuelingByVehicle.map((_, index) => <Cell key={`cell-${index}`} fill={VEHICLE_COLORS[index % VEHICLE_COLORS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer> : <div className="h-[250px] flex items-center justify-center text-muted-foreground">
                  <p className="text-sm">Nenhum dado disponível</p>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              {data?.monthlyRefueling && data.monthlyRefueling.some(m => m.count > 0) ? <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={data.monthlyRefueling}>
                    <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                    <XAxis dataKey="monthName" tick={{
                  fontSize: 10
                }} axisLine={false} angle={-45} textAnchor="end" height={60} />
                    <YAxis tick={{
                  fontSize: 12
                }} axisLine={false} tickFormatter={formatLiters} label={{
                  value: 'Litros',
                  angle: -90,
                  position: 'insideLeft',
                  style: {
                  ...
                  return [value, name];
                }} labelFormatter={label => `Mês: ${label}`} contentStyle={{
                  backgroundColor: "hsl(var(--card))",
                  border: "1px solid hsl(var(--border))",
                  borderRadius: "8px"
                }} />
        ...
        {/* Daily Details Table */}
        {data?.dailyByVehicle && data.dailyByVehicle.length > 0 && <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-semibold">
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.dailyByVehicle.sort((a, b) => b.date.localeCompare(a.date)).map((item, index) => <tr key={index} className="border-b last:border-b-0 hover:bg-muted/50">
                  ...
                  <tfoot>
                    <tr className="bg-muted/30 font-medium">
                      ...
                      <td className="py-2 px-3 text-right text-cyan-600 dark:text-cyan-400">
                        {data.dailyByVehicle.reduce((sum, item) => sum + item.liters, 0).toLocaleString("pt-BR")} L
                      </td>
                    </tr>
                  </tfoot>
                </table>
