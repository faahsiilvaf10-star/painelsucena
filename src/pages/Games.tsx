...
import { Gamepad2, Trophy, RotateCcw, Star, Check, X, Timer, Leaf, Zap } from "lucide-react";
...
export default function Games() {
  ...
  const handleAnswer = useCallback((chosenBin: BinType | "timeout") => {
    ...
    const message = isTimeout
      ? `⏰ Tempo esgotado! Era: ${correctBinLabel}`
      : correct
        ? `✅ Correto! +${points} pontos`
        : `❌ Errado! Era: ${correctBinLabel}`;
      ...
      <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Jogos educativos sobre meio ambiente</p>
          </div>
        </div>
        ...
        {gameState === "playing" && currentItem && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {streak > 1 && (
                <Badge className="gap-1 text-sm bg-amber-500/90 text-white">
                  🔥 {streak}x combo
                </Badge>
              )}
                  ...
                  <CardContent className="p-6 text-center space-y-3">
                    ...
                    {feedback && (
                      <motion.div
                        ...
                        className={`text-sm font-semibold ${feedback.correct ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                      >
                        {feedback.message}
                      </motion.div>
                    )}
            ...
            {!showHint && !feedback && (
              <div className="text-center">
                <Button variant="ghost" size="sm" onClick={() => setShowHint(true)} className="text-xs text-muted-foreground">
                  💡 Mostrar dica (-5 pts)
                </Button>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              {BINS.map((bin) => (
                <Button
                  ...
                  disabled={!!feedback}
                  onClick={() => {
                    if (showHint) setScore((prev) => Math.max(0, prev - 5));
                    handleAnswer(bin.type);
                  }}
                  className={`h-auto py-4 flex flex-col gap-1 border-2 transition-all hover:scale-[1.02] active:scale-95 ${bin.borderClass} ${bin.bgClass}`}
                >
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-center gap-1">
                  {[1, 2, 3].map((star) => (
                    <Star
                      key={star}
                      className={`w-8 h-8 ${star <= getStarRating() ? "text-amber-400 fill-amber-400" : "text-muted"}`}
                    />
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  ...
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="text-2xl font-bold text-green-600">{answers.filter(a => a.correct).length}/{ROUND_SIZE}</div>
                    <div className="text-xs text-muted-foreground">Acertos</div>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="text-2xl font-bold text-amber-500">{bestStreak}x</div>
                    <div className="text-xs text-muted-foreground">Melhor Combo</div>
                  </div>
                </div>
                    ...
                    {answers.map((answer, i) => {
                      ...
                      return (
                        <div key={i} className={`flex items-center gap-2 p-2 rounded-lg text-sm ${answer.correct ? "bg-green-500/10" : "bg-red-500/10"}`}>
                          ...
                          {!answer.correct && (
                            <span className="text-xs text-muted-foreground">→ {correctBin?.label}</span>
                          )}
                        </div>
                      );
                    })}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        A separação correta do lixo reduz em até 30% o volume de resíduos em aterros e permite a reciclagem de materiais valiosos. No canteiro de obras, siga sempre as orientações do PGRS!
                      </p>
                    </div>
                  </div>
                </div>
