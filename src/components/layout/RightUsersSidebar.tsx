...
import { Users, ChevronRight, ChevronLeft, MessageCircle } from "lucide-react";
...
export const RightUsersSidebar = ({ onUserClick }: RightUsersSidebarProps) => {
  ...
  const onlineUsers = allUsers.filter((u) => u.isOnline && !u.isCurrentUser);
  const offlineUsers = allUsers.filter((u) => !u.isOnline && !u.isCurrentUser);
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          onClick={onClick}
          className="w-full flex items-center gap-2 px-1.5 py-1.5 rounded-md hover:bg-muted/60 transition-colors group text-left"
        >
