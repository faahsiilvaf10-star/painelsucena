...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Fetch Responsible User Profile
    let responsiblePhone = "";
    if (desvio.mentioned_user_id) {
      const { data: profile } = await admin
        .from("profiles")
        .select("whatsapp_number")
        .eq("user_id", desvio.mentioned_user_id)
        .maybeSingle();
      if (profile?.whatsapp_number) {
        responsiblePhone = profile.whatsapp_number.replace(/\D/g, "");
      }
    }
    ...
    if (responsiblePhone) {
      tasks.push(queueMessage(responsiblePhone, false));
    }
